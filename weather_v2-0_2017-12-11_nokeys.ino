...
// ****** Sensors ******
boolean sense_dht = false; // temp/humidity sensor
boolean sense_photo = false; // photoresistor light sensor
boolean sense_tsl = false; // tsl lux sensor
boolean sense_rain = false; // tipping bucket rain gauge
boolean sense_wind = false; // anemometer sensor
...
// ****** Testing ******
boolean testing = true; // Uses benchTime instead of initialTime
...
void loop() {
    ...
    if (first_loop == true) {
        delay(5000); // Give user 5 seconds to initialize serial connection
    ...
    if (Time.now() >= scheduledPublish) {
        if ((cloud_connect == true) && (Particle.connected() == false)) {
            Serial.print(F("What the!"));
            Serial.println(String(publish_particle));
            Serial.println(String(publish_thingspeak));
            Serial.println(String(publish_wunderground));
            Serial.println(String(cloud_connect));
            Particle.connect();
        }
        ...
        if (publish_serial == true) {
            if (units_imperial == false) {
                publishToSerial_metric(tempC,humidityRH,light,rainToday_mm,wind_mps,wind_gust,soil_temp_C,sol_rad);
            }
            else if (units_imperial == true) {
                publishToSerial_imperial(temp_f,humidityRH,light,rainToday_in,wind_mph,windGust_mph,soil_temp_F,sol_rad);
            }
        }
        ...
        if (publish_sd == true) {
            if (units_imperial == false) {
                publishToSD(hhmmss(scheduledPublish),tempC,humidityRH,light,lux,rainToday_mm,wind_mps,wind_gust,wind_dir,soilMoisture,soil_temp_C,sol_rad);
            }
            else if (units_imperial == true) {
                publishToSD(hhmmss(scheduledPublish),temp_f,humidityRH,light,lux,rainToday_in,wind_mph,windGust_mph,wind_dir,soilMoisture,soil_temp_F,sol_rad);
            }
        }
...
float getAndResetSolrad() {
    ...
    float result = (float(solradTotal)/float(solradReadingCount));  //This was originally 4096, but I noticed light levels never got below 50%
    solradTotal = 0.0;
...
// ============================================================
void publishToSerial_metric(float tempC,float humidityRH,float light,float rain_mm,float wind_mps,float wind_gust,float soil_temp_C,float sol_rad) {
    ...
    Serial.print(F("Solar Radiation: ")); Serial.print(sol_rad); Serial.println(F(" W m-2"));
    Serial.println();
}

void publishToSerial_imperial(float temp_f,float humidityRH,float light,float rain_in,float wind_mph,float wind_gust_mph, float soil_temp_F,float sol_rad) {
...
void publishToSD(String datetime,float temp,float humidityRH,float light,float lux,float rainToday,float wind,float wind_gust,float wind_dir,float soil_cbar,float soil_temp,float sol_rad) {
    String data_string = datetime + ",";
    if (sense_dht == true){
        data_string += String(temp) + "," + String(humidityRH) + ",";
    }
    if (sense_photo == true) {
        data_string += String(light) + ",";
    }
    if (sense_tsl == true) {
        data_string += String(lux) + ",";
    }
    if (sense_rain == true) {
        data_string += String(rainToday) + ",";
    }
    if (sense_wind == true) {
        data_string += String(wind) + "," + String(wind_gust) + ",";
    }
    if (sense_dir == true) {
        data_string += String(wind_dir) + ",";
    }
    if (sense_moist == true){
        data_string += String(soil_cbar) + ",";
    }
    if (sense_soiltemp == true) {
        data_string += String(soil_temp) + ",";
    }
    if (sense_solrad == true) {
        data_string += String(sol_rad) + ",";
    }
    SoftSer.print(data_string);
}

data_string

SoftSer.print(scheduledPublish,tempC,humidityRH,light,rainToday_mm,wind_mps,wind_gust,soil_temp_C,sol_rad);
SoftSer.print(scheduledPublish,temp_f,humidityRH,light,rainToday_in,wind_mph,windGust_mph,soil_temp_F,sol_rad);
